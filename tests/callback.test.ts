...
test("classifica retornos ausentes, negados, malformados e duplicados", () => {
  const cases = [
    ...
    ["state=example", "invalid_request"],
    ["code=a&state=b&state=c", "invalid_request"],
    ["code=%0A&state=a", "invalid_request"],
    ["code=a&state=fake-or-expired", "integration_disabled"],
  ];
  for (const [query, expected] of cases)
    assert.equal(classifyCallback(new URLSearchParams(query)), expected);
});
test("callback limpa code/state e não chama nenhum transporte externo", async () => {
  const original = globalThis.fetch;
  globalThis.fetch = (() => {
    throw new Error("Nenhuma chamada externa é permitida no callback demo");
  }) as typeof fetch;
  try {
    for (const query of [
      "",
      ...
      const response = await GET(
        new Request(
          `https://untrusted-host.example/api/integrations/bling/callback${query}`,
        ),
      );
      assert.equal(response.status, 303);
      const location = response.headers.get("Location")!;
      assert.ok(location.startsWith("/integracao/retorno?resultado="));
      assert.ok(!location.includes("private"));
      assert.ok(!location.includes("untrusted-host"));
...
test("iniciar conexão é bloqueado por código, não por presença de variável", async () => {
  const response = await POST(
    new Request("https://example.com/api/integrations/bling/connect", {
      method: "POST",
    }),
  );
