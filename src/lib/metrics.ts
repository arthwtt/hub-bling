...
export function selectOrders(orders: Order[], filters: Filters) {
  return orders.filter(
    (order) =>
      order.date >= filters.start &&
      order.date <= filters.end &&
      (filters.channel === "Todos" || order.channel === filters.channel),
  );
}
export function summarize(data: DemoData, filters: Filters) {
  ...
  const days = Math.max(
    1,
    Math.round(
      (Date.parse(filters.end) - Date.parse(filters.start)) / 86400000,
    ) + 1,
  );
  const ranking = data.products
    .map((product) => {
      ...
      return {
        ...
        sales,
        available,
        coverage: quantity > 0 ? available / (quantity / days) : null,
      };
    })
  ...
  const customers = data.customers
    .map((customer) => {
      const purchases = eligible.filter(
        (order) => order.customerId === customer.id,
      );
      return {
        ...customer,
        count: purchases.length,
        total: purchases.reduce((sum, order) => sum + order.total, 0),
        last:
          purchases
  ...
  return {
    ...
    revenue,
    units,
    ticket: eligible.length ? Math.round(revenue / eligible.length) : null,
    products,
    customers,
    daily,
    canceled: all.length - eligible.length,
  };
}
export function summarizeBills(data: DemoData, start: string, end: string) {
  ...
  const bills = data.bills.filter(
    (bill) => bill.due >= start && bill.due <= end,
  );
  const active = bills.filter(
    (bill) => bill.status !== "Cancelado" && bill.status !== "Liquidado",
  );
  const receivable = active
    .filter((b) => b.kind === "receber")
    .reduce((sum, b) => sum + b.balance, 0);
  const payable = active
    .filter((b) => b.kind === "pagar")
    .reduce((sum, b) => sum + b.balance, 0);
  const overdue = active
    .filter((b) => b.kind === "receber" && b.due < DEMO_TODAY)
    .reduce((sum, b) => sum + b.balance, 0);
  return {
    bills,
    receivable,
