...
const raw = readFileSync(new URL('./bling-openapi.json', import.meta.url), 'utf8');
const spec = JSON.parse(raw.replace(/^\uFEFF/, ''));
const resolve = (ref) => ref.split('/').slice(1).reduce((value, key) =>
  value?.[key.replace(/~1/g, '/').replace(/~0/g, '~')], spec);
...
const operations = Object.entries(spec.paths).flatMap(([path, item]) =>
  Object.entries(item).filter(([method]) => methods.has(method))
    .map(([method, op]) => ({ path, method, op,
      parameters: [...(item.parameters ?? []), ...(op.parameters ?? [])].map(deref) })));
const issues = [];
function walk(value, path = '#') {
  ...
  if (value.properties && Array.isArray(value.required) && !value.allOf && !value.oneOf)
    for (const key of value.required)
      if (!(key in value.properties)) issues.push(`required sem propriedade local: ${path} -> ${key}`);
...
for (const { path, method, parameters } of operations) {
  ...
  for (const p of parameters.filter(Boolean)) {
    ...
    if (seen.has(id)) issues.push(`Parâmetro duplicado: ${method.toUpperCase()} ${path} -> ${id}`);
    seen.add(id);
  }
}
...
const report = [
  ...
  '|---|---|---|---|',
  ...get.map(({ path, op, parameters }) => `| \`${path}\` | ${esc(op.tags?.join(', '))} | ${esc(parameters.map(p => p?.name).filter(Boolean).join(', ')) || '—'} | ${Object.keys(op.responses ?? {}).join(', ')} |`), '',
...
writeFileSync(new URL('./API-AUDITORIA.md', import.meta.url), report.join('\n'));
console.log(JSON.stringify({ paths: Object.keys(spec.paths).length, operations: operations.length,
  get: get.length, schemas: Object.keys(spec.components.schemas).length, structuralFindings: issues.length }));
